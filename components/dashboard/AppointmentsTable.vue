      ...
      <table v-else class="border p-10 rounded-lg w-full text-left table-collapse">
        ...
        <tbody>
          <tr class="w-fit p-4 font-thin" v-for="patient in filteredPatients" :key="patient.id">
            ...
            <td v-if="role === 'PATIENT'" class="p-2 pt-3 text-sm  border-t border-gray-100 text-gray-700">{{ patient.patient.mobile}}</td>
...
<button v-else @click="Call(patient.patient.mobile)" class="btn text-lg bg-white text-blue-700"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-phone" width="19" height="19" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
   ...
   <path d="M9 7v-3a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v3"></path>
                ...
                <button v-if="role !== 'PATIENT'" @click="modal = true" class="btn text-lg bg-white text-blue-700"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-eye" width="19" height="19" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
   <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
...
<div v-if="this.modal" class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
  ...
  async markAsDone(id){
    try{
        await axios.put('https://hit200-group8.azurewebsites.net/v1/appointments/mark_done_appointment?id='+id,{
        },{
...
console.log("Error:",err.message)
  ...
  async DeleteRecord(id){
    try{
        await axios.delete('https://hit200-group8.azurewebsites.net/v1/appointments/delete/'+id,{
        id:id,
        },{
            headers: {'Content-Type': 'application/json',
...
console.log("Error:",err.message)
  ...
  async addAppointmentData(id,patient){
       ...
       try{
        await axios.post('https://hit200-group8.azurewebsites.net/v1/appointments_data/create?patientId='+patient,{
          appointment: {id:id}
          // params:{
          //   doctorId:this.id,
          //   start:this.date
